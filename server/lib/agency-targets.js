...
async function listDesignatedPlaylistsAsync(db, tokenId, workspaceId) {
  return db.prepare(`
    SELECT p.id, p.name, p.status FROM api_token_targets t
    JOIN playlists p ON p.id = t.playlist_id
    WHERE t.token_id = ? AND p.workspace_id = ? ORDER BY p.name
  `).all(tokenId, workspaceId);
}
...
module.exports = { listDesignatedPlaylists, listDesignatedPlaylistsAsync, isZonedPlaylist, isZonedPlaylistAsync };
