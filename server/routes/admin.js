...
const { canAdminWorkspaceAsync } = require('../lib/permissions');
const { requirePlatformAdmin } = require('../middleware/auth');
const { logActivity, getClientIp } = require('../services/activity');
const { deleteWorkspaceCascadeAsync, deleteOrgCascadeAsync } = require('../lib/user-deletion');
const { resolveBrandingAsync, HARDCODED_BRANDING, PLATFORM_DEFAULT_ID } = require('../lib/branding');
...
router.post('/users', routeAsync(async (req, res) => {
...
router.post('/orgs', requirePlatformAdmin, routeAsync(async (req, res) => {
  const name = String(req.body?.name || '').trim();
  ...
  const txn = db.transaction(async () => {
    ...
    await db.prepare(`INSERT INTO organization_members (organization_id, user_id, role) VALUES (?, ?, 'org_owner')`).run(orgId, ownerId);
    await db.prepare(`INSERT INTO workspaces (id, organization_id, name, created_by) VALUES (?, ?, 'Default', ?)`).run(wsId, orgId, ownerId);
    await db.prepare(`INSERT INTO workspace_members (workspace_id, user_id, role) VALUES (?, ?, 'workspace_admin')`).run(wsId, ownerId);
  });
  await txn();
...
router.get('/orgs', requirePlatformAdmin, routeAsync(async (req, res) => {
...
router.delete('/orgs/:id', requirePlatformAdmin, routeAsync(async (req, res) => {
  const org = await db.prepare('SELECT id, name FROM organizations WHERE id = ?').get(req.params.id);
  if (!org) return res.status(404).json({ error: 'Organization not found' });
  try {
    await deleteOrgCascadeAsync(db, { orgId: org.id });
  } catch (e) {
...
router.delete('/workspaces/:id', requirePlatformAdmin, routeAsync(async (req, res) => {
  const ws = await db.prepare('SELECT id, name, organization_id FROM workspaces WHERE id = ?').get(req.params.id);
  if (!ws) return res.status(404).json({ error: 'Workspace not found' });
  try {
    await deleteWorkspaceCascadeAsync(db, { workspaceId: ws.id });
  } catch (e) {
...
router.put('/users/:id/workspace', requirePlatformAdmin, routeAsync(async (req, res) => {
  ...
  const memberships = await db.prepare('SELECT workspace_id FROM workspace_members WHERE user_id = ?').all(target.id);
  if (memberships.length > 1) {
    return res.status(400).json({ error: 'User belongs to multiple workspaces - manage in the workspace members view' });
  }

  const ws = await db.prepare('SELECT id, name, organization_id FROM workspaces WHERE id = ?').get(workspaceId);
  if (!ws) return res.status(404).json({ error: 'Workspace not found' });
  ...
  if (memberships.length === 1 && memberships[0].workspace_id === ws.id) {
    const cur = await db.prepare('SELECT role FROM workspace_members WHERE user_id = ? AND workspace_id = ?').get(target.id, ws.id);
  ...
  const txn = db.transaction(async () => {
    await db.prepare('DELETE FROM workspace_members WHERE user_id = ?').run(target.id);
    await db.prepare('INSERT INTO workspace_members (workspace_id, user_id, role, invited_by) VALUES (?, ?, ?, ?)')
...
router.get('/users/:id/workspaces', requirePlatformAdmin, routeAsync(async (req, res) => {
...
router.post('/users/:id/workspaces', requirePlatformAdmin, routeAsync(async (req, res) => {
  const role = String(req.body?.role || '').trim();
  const workspaceId = String(req.body?.workspaceId || '').trim();
  if (!workspaceId) return res.status(400).json({ error: 'workspaceId required' });
  if (!WORKSPACE_ROLES.includes(role)) {
  ...
  const existing = await db.prepare('SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?').get(ws.id, target.id);
  if (existing) {
    await db.prepare('UPDATE workspace_members SET role = ? WHERE workspace_id = ? AND user_id = ?').run(role, ws.id, target.id);
  } else {
    await db.prepare('INSERT INTO workspace_members (workspace_id, user_id, role, invited_by) VALUES (?, ?, ?, ?)').run(ws.id, target.id, role, req.user.id);
  }
  await logActivity(req.user.id, 'admin_add_user_workspace', `target: ${target.email}, workspace: ${ws.id}, role: ${role}`, null, getClientIp(req), ws.id);
  const org = await db.prepare('SELECT name FROM organizations WHERE id = ?').get(ws.organization_id);
...
router.put('/users/:id/workspaces/:workspaceId', requirePlatformAdmin, routeAsync(async (req, res) => {
  const role = String(req.body?.role || '').trim();
  if (!WORKSPACE_ROLES.includes(role)) {
    return res.status(400).json({ error: 'Role must be workspace_admin, workspace_editor, or workspace_viewer' });
  }
  const member = await db.prepare('SELECT 1 FROM workspace_members WHERE workspace_id = ? AND user_id = ?').get(req.params.workspaceId, req.params.id);
  if (!member) return res.status(404).json({ error: 'Membership not found' });
  await db.prepare('UPDATE workspace_members SET role = ? WHERE workspace_id = ? AND user_id = ?').run(role, req.params.workspaceId, req.params.id);
  req.workspaceId = req.params.workspaceId;
  const target = await db.prepare('SELECT email FROM users WHERE id = ?').get(req.params.id);
  await logActivity(req.user.id, 'admin_set_user_workspace_role', `target: ${target?.email}, workspace: ${req.params.workspaceId}, role: ${role}`, null, getClientIp(req), req.params.workspaceId);
...
router.delete('/users/:id/workspaces/:workspaceId', requirePlatformAdmin, routeAsync(async (req, res) => {
  const member = await db.prepare('SELECT 1 FROM workspace_members WHERE workspace_id = ? AND user_id = ?').get(req.params.workspaceId, req.params.id);
  if (!member) return res.status(404).json({ error: 'Membership not found' });
  await db.prepare('DELETE FROM workspace_members WHERE workspace_id = ? AND user_id = ?').run(req.params.workspaceId, req.params.id);
  req.workspaceId = req.params.workspaceId;
  const target = await db.prepare('SELECT email FROM users WHERE id = ?').get(req.params.id);
  await logActivity(req.user.id, 'admin_remove_user_workspace', `target: ${target?.email}, workspace: ${req.params.workspaceId}`, null, getClientIp(req), req.params.workspaceId);
...
router.get('/branding', requirePlatformAdmin, routeAsync(async (req, res) => {
  res.json(await resolveBrandingAsync(db) || { ...HARDCODED_BRANDING });
}));

// PUT - upsert the single platform-default row (workspace_id IS NULL).
router.put('/branding', requirePlatformAdmin, routeAsync(async (req, res) => {
  const existing = await db.prepare('SELECT * FROM white_labels WHERE id = ?').get(PLATFORM_DEFAULT_ID);
  if (existing) {
    const updates = [];
    const values = [];
    ...
    if (updates.length) {
      ...
      await db.prepare(`UPDATE white_labels SET ${updates.join(', ')} WHERE id = ?`).run(...values);
    }
  } else {
  ...
  await logActivity(req.user.id, 'admin_set_platform_branding', `brand: ${req.body.brand_name || ''}`, null, getClientIp(req), null);
  res.json(await db.prepare('SELECT * FROM white_labels WHERE id = ?').get(PLATFORM_DEFAULT_ID));
}));
