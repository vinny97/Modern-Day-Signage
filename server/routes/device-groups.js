...
async function loadGroupAccessCtx(req, res) {
  const group = await db.prepare('SELECT * FROM device_groups WHERE id = ?').get(req.params.id);
  ...
  const ws = await db.prepare('SELECT * FROM workspaces WHERE id = ?').get(group.workspace_id);
  const ctx = ws && await accessContextAsync(req.user.id, req.user.role, ws);
  if (!ctx) { res.status(403).json({ error: 'Access denied' }); return null; }
  return { group, ctx };
}

const requireGroupRead = routeAsync(async (req, res, next) => {
  const access = await loadGroupAccessCtx(req, res);
  if (!access) return;
  req.group = access.group;
  next();
});

const requireGroupWrite = routeAsync(async (req, res, next) => {
  const access = await loadGroupAccessCtx(req, res);
  if (!access) return;
...
router.post('/', routeAsync(async (req, res) => {
  ...
  const id = uuidv4();
  await db.prepare('INSERT INTO device_groups (id, user_id, workspace_id, name, color) VALUES (?, ?, ?, ?, ?)')
    .run(id, req.user.id, req.workspaceId, name, color || '#3B82F6');
  res.status(201).json(await db.prepare('SELECT * FROM device_groups WHERE id = ?').get(id));
}));

// Update group
router.put('/:id', requireGroupWrite, routeAsync(async (req, res) => {
  const { name, color } = req.body;
  if (color && !VALID_COLOR.test(color)) return res.status(400).json({ error: 'invalid color format, use #RRGGBB' });
  if (name) await db.prepare('UPDATE device_groups SET name = ? WHERE id = ?').run(name, req.params.id);
  if (color) await db.prepare('UPDATE device_groups SET color = ? WHERE id = ?').run(color, req.params.id);
  res.json(await db.prepare('SELECT * FROM device_groups WHERE id = ?').get(req.params.id));
}));

// Delete group — converts group schedules to per-device schedules first
router.delete('/:id', requireGroupWrite, routeAsync(async (req, res) => {
  const groupId = req.params.id;

  const convert = db.transaction(async () => {
    // Find group schedules that need conversion
    const groupSchedules = await db.prepare('SELECT * FROM schedules WHERE group_id = ?').all(groupId);

    // Find current group members
    const members = await db.prepare('SELECT device_id FROM device_group_members WHERE group_id = ?').all(groupId);
    ...
    await db.prepare('DELETE FROM schedules WHERE group_id = ?').run(groupId);

    // Delete the group (cascades to device_group_members)
    await db.prepare('DELETE FROM device_group_members WHERE group_id = ?').run(groupId);
    await db.prepare('DELETE FROM device_groups WHERE id = ?').run(groupId);
...
router.get('/:id/devices', requireGroupRead, routeAsync(async (req, res) => {
...
router.post('/:id/devices', requireGroupWrite, routeAsync(async (req, res) => {
  ...
  try {
    await db.prepare('INSERT OR IGNORE INTO device_group_members (device_id, group_id) VALUES (?, ?)').run(device_id, req.params.id);
    ...
    const group = await db.prepare('SELECT playlist_id FROM device_groups WHERE id = ?').get(req.params.id);
    const newPlaylist = group?.playlist_id || null;
    await db.prepare('UPDATE devices SET playlist_id = ? WHERE id = ?').run(newPlaylist, device_id);
    await pushPlaylistToDevice(req, device_id);
...
router.delete('/:id/devices/:deviceId', requireGroupWrite, routeAsync(async (req, res) => {
  ...
  await db.prepare('UPDATE devices SET playlist_id = ? WHERE id = ?').run(newPlaylist, deviceId);
  await pushPlaylistToDevice(req, deviceId);
...
async function pushPlaylistToDevice(req, deviceId) {
  try {
...
router.post('/:id/assign-content', requireGroupWrite, routeAsync(async (req, res) => {
  ...
  const members = await db.prepare('SELECT device_id FROM device_group_members WHERE group_id = ?').all(req.params.id);

  const transaction = db.transaction(async () => {
    for (const m of members) {
      const playlistId = await ensureDevicePlaylist(m.device_id, req.user.id);
      const max = await db.prepare('SELECT COALESCE(MAX(sort_order),0)+1 as next FROM playlist_items WHERE playlist_id = ?').get(playlistId);
      await db.prepare('INSERT INTO playlist_items (playlist_id, content_id, sort_order, duration_sec) VALUES (?, ?, ?, ?)')
...
router.post('/:id/assign-playlist', requireGroupWrite, routeAsync(async (req, res) => {
  const { playlist_id } = req.body;
  if (!playlist_id) return res.status(400).json({ error: 'playlist_id required' });

  const playlist = await db.prepare('SELECT id, workspace_id FROM playlists WHERE id = ?').get(playlist_id);
  ...
  const transaction = db.transaction(async () => {
    await db.prepare('UPDATE device_groups SET playlist_id = ? WHERE id = ?').run(playlist_id, req.params.id);
    for (const m of members) await stmt.run(playlist_id, m.device_id);
  });
  await transaction();

  for (const m of members) await pushPlaylistToDevice(req, m.device_id);
  res.json({ success: true, devices_updated: members.length });
}));

// Send command to all devices in a group (reboot/shutdown/screen on/off etc.)
router.post('/:id/command', requireScope('full'), requireGroupWrite, routeAsync(async (req, res) => {
