...
const { isZonedPlaylistAsync } = require('../lib/agency-targets'); // #73: full-screen-only guardrail
...
router.post('/', routeAsync(async (req, res) => {
  ...
  if (!await accessContextAsync(req.user.id, 'user', req.workspace)) {
  ...
  if (scope === 'agency') {
    ...
    for (const pid of targetIds) {
      ...
      if (await isZonedPlaylistAsync(db, pid)) return res.status(400).json({ error: 'A selected playlist is assigned to a zone on a screen — agency uploads play full-screen, so it can\'t be shared with an agency. Use a full-screen playlist.' });
    }
  }
...
router.delete('/:id', routeAsync(async (req, res) => {
  ...
  if (!row.revoked_at) {
    await db.prepare("UPDATE api_tokens SET revoked_at = strftime('%s','now') WHERE id = ?").run(req.params.id);
  }
...
router.put('/:id/targets', routeAsync(async (req, res) => {
  ...
  const replace = db.transaction(async () => {
    await db.prepare('DELETE FROM api_token_targets WHERE token_id = ?').run(tok.id);
