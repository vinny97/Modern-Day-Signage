...
router.get('/', routeAsync(async (req, res) => {
  res.json(await resolveBrandingAsync(db, { workspaceId: req.workspaceId || null }));
}));
...
router.get('/domain/:domain', routeAsync(async (req, res) => {
  res.json(publicBranding(await resolveBrandingAsync(db, { domain: req.params.domain })));
}));
...
router.post('/', requireWorkspaceAdmin, routeAsync(async (req, res) => {
  ...
  let wl = await db.prepare('SELECT * FROM white_labels WHERE workspace_id = ?').get(req.workspaceId);
  ...
  if (wl) {
    ...
    if (updates.length) {
      ...
      await db.prepare(`UPDATE white_labels SET ${updates.join(', ')} WHERE workspace_id = ?`).run(...values);
    }
  } else {
    const id = uuidv4();
    await db.prepare(`INSERT INTO white_labels (id, user_id, workspace_id, brand_name, logo_url, favicon_url, primary_color, secondary_color, bg_color, custom_domain, custom_css, hide_branding)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
  ...
  res.json(await db.prepare('SELECT * FROM white_labels WHERE workspace_id = ?').get(req.workspaceId));
}));
